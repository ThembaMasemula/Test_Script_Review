...
public class GlobalAssignmentInfo extends BaseTestSecureValidation {
	...
	private void preTest(Integer profile) throws Exception {
		...
		try {
			employeeProfilePage.clickCompensationLabel();
	...
	private void securityViewHistory() throws Exception {
		System.out.println("Verify History");
		fw.waitForLoadPage();
		employeeProfilePage.clickHistoryGlobalAssignmentInfo();
	...
	private void securityAddData() throws Exception {
		...
		fw.waitForLoadPage();
		employeeProfilePage.clickEditGlobalAssignmentInfo();
