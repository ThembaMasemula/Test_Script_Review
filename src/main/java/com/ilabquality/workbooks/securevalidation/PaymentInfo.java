...
public class PaymentInfo extends BaseTestSecureValidation {
	...
	private void preTest(Integer profile) throws Exception {
		...
		try {
			employeeProfilePage.setFocusPaymentInformation();
	...
	private void securityViewHistory() throws Exception {
		System.out.println("Verify History");
		fw.waitForLoadPage();
		employeeProfilePage.clickHistoryPaymentInformation();
	...
	private void securityAddData() throws Exception {
		...
		fw.waitForLoadPage();
		employeeProfilePage.clickEditPaymentInformation();
