...
public class EmploymentInfoAtHire extends BaseTestSecureValidation {
	...
	private void preTest(Integer profile) throws Exception {
		...
		try {
			employeeProfilePage.clickEmploymentLabel();
	...
	private void securityViewHistory() throws Exception {
		System.out.println("Verify History");
		fw.waitForLoadPage();
		employeeProfilePage.clickHistoryPositionInformation();
	...
	private void securityAddData() throws Exception {
		...
		fw.waitForLoadPage();
		employeeProfilePage.clickEditPositionInformation();
