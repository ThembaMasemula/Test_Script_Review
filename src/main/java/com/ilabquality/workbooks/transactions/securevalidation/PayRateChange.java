...
public class PayRateChange extends BaseTestSecureValidationHRTransactions {
	...
	public static void beforeEach() throws EncryptedDocumentException, IOException {
		...
		if (!parametersOk) {
			parameters = data("Pay Rate Change");
			parametersOk = true;
		}
		// System.out.println("OK: " + parametersOk);
	}
	...
	public void test() throws Exception {
		...
		try {
			...
			// changeJobCompensationInfoPage.waitUntilLoadingDisappear();
			// fw.timeout(1000);
			// String codePayGrade = payGradeOld.substring(payGradeOld.indexOf("(") +1,
			// payGradeOld.indexOf(")"));
			// Integer code =
			// Integer.parseInt(codePayGrade.substring(codePayGrade.indexOf("-") +1,
			// codePayGrade.length())) + 1;
			// changeJobCompensationInfoPage.setPayGrade(payGradeOld.substring(payGradeOld.indexOf("(")
			// +1, payGradeOld.indexOf("-"))+"-" + "0"+code.toString());
			// fw.timeout(1000);
			// changeJobCompensationInfoPage.waitUntilLoadingDisappear();
