...
public class PartTimeToFullTime extends BaseTestSecureValidationHRTransactions {
	...
	public static void beforeEach() throws EncryptedDocumentException, IOException {
		...
		if (!parametersOk) {
			parameters = data("Part Time To Full Time");
			parametersOk = true;
		}
		// System.out.println("OK: " + parametersOk);
	}
	...
	public void test() throws Exception {
		...
		try {
			...
			changeJobCompensationInfoPage.setFirstJobClassification();
			fw.timeout(3000);
			changeJobCompensationInfoPage.waitUntilLoadingDisappear();
			fw.timeout(1000);
			String codePayGrade = payGradeOld.substring(payGradeOld.indexOf("(") + 1, payGradeOld.indexOf(")"));
			changeJobCompensationInfoPage.setPayGrade(codePayGrade);
			fw.timeout(1000);
			changeJobCompensationInfoPage.waitUntilLoadingDisappear();
			changeJobCompensationInfoPage.setIsFulltimeEmployee("Y");
			changeJobCompensationInfoPage.waitUntilLoadingDisappear();
			// fw.getFullScreenshot();
